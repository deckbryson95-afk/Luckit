...
const require_sql = require("../../sql-BeWZKBJp.cjs");
...
//#region src/middleware/postgres/index.ts
/**
...
*/
const SUPPORTED_ROLES = /* @__PURE__ */ new Set(["authenticated", "anon"]);
/**
...
*/
function resolveRole(claims, errors) {
	const requested = claims?.role;
	if (requested == null) return "anon";
	if (typeof requested === "string" && SUPPORTED_ROLES.has(requested)) return requested;
	return require_error_response.errorResponse(require_errors.Errors[require_errors.UnsupportedRoleError]({
...
* @category Middleware
*/
const withPostgresClient = (0, _supabase_middleware.defineMiddleware)({
	key: "postgres",
	run: (config) => async (_req, ctx) => {
		const connectionString = require_sql.resolveConnectionString(config?.connectionString);
		if (!connectionString) return require_sql.missingConnectionStringResponse("withPostgresClient", config?.errors);
		const claims = ctx.jwtClaims;
		const role = resolveRole(claims, config?.errors);
		if (role instanceof Response) return role;
		const p = require_sql.getPool(connectionString);
		const claimsJson = JSON.stringify(claims ?? {});
		const api = {
			query(strings, ...values) {
				const compiled = require_sql.compileTemplate(strings, values);
				return api.queryRaw(compiled.text, compiled.values);
			},
			async queryRaw(text, params) {
				const client = await p.connect();
				let poisoned = false;
				try {
					await client.query("begin");
					await client.query(`select set_config('request.jwt.claims', $1, true)`, [claimsJson]);
					await client.query(`set local role ${require_sql.ident(role)}`);
					...
					if (e instanceof Error && e.code === "42501") e.message += ` (RLS-scoped queries run as the caller's role '${role}' — grant that role the table privileges it needs, e.g. "grant select on <table> to ${role}")`;
					throw e;
				} finally {
...
//#endregion
exports.ident = require_sql.ident;
