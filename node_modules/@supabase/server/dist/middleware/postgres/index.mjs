import { T as UnsupportedRoleError, c as Errors } from "../../errors-DV038ZPa.mjs";
import { t as errorResponse } from "../../error-response-DAWGVKnt.mjs";
import { a as resolveConnectionString, i as missingConnectionStringResponse, n as ident, r as getPool, t as compileTemplate } from "../../sql-CXjoX0mk.mjs";
...
function resolveRole(claims, errors) {
	...
	return errorResponse(Errors[UnsupportedRoleError]({
...
* @category Middleware
*/
const withPostgresClient = defineMiddleware({
	key: "postgres",
	run: (config) => async (_req, ctx) => {
		const connectionString = resolveConnectionString(config?.connectionString);
		if (!connectionString) return missingConnectionStringResponse("withPostgresClient", config?.errors);
...
//#endregion
export { ident, withPostgresClient };
