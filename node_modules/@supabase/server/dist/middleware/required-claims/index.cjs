...
//#region src/middleware/required-claims/index.ts
/**
...
* @category Middleware
*/
const withRequiredClaims = (0, require("@supabase/middleware").defineMiddleware)({
	key: "jwtClaims",
	run: (config) => async (req) => {
		const { token, apikey } = require_extract_credentials.extractCredentials(req);
		const diagnosis = require_classify_credentials.diagnoseAuthorizationHeader(req.headers.get("authorization"));
		if (diagnosis.kind !== "bearer" || !token) {
			const received = {
				authorization: diagnosis.kind === "unreadable" ? "non-bearer-scheme" : diagnosis.kind === "api-key" ? "api-key" : "absent",
				apikey: require_classify_credentials.classifyApiKey(apikey)
			};
			return require_error_response.errorResponse(diagnosis.kind === "absent" ? require_errors.Errors[require_errors.MissingCredentialsError]({
				authModes: ["user"],
				received
			}) : require_errors.Errors[require_errors.UnusableCredentialError]({
				authModes: ["user"],
				received,
		...
		const jwks = config?.jwks ?? require_resolve_env.resolveJwks();
		if (!jwks) return require_error_response.errorResponse(require_errors.Errors[require_errors.JwksNotConfiguredError]({ middleware: "withRequiredClaims" }), { errors: config?.errors });
		const verified = await require_verify_user_jwt.verifyUserJwt(token, jwks, {
			audience: config?.audience,
			issuer: config?.issuer
		});
		if (!verified.ok) {
			const { failure } = verified;
			return require_error_response.errorResponse(failure.kind === "jwks-source" ? require_errors.Errors[require_errors.JwksFetchFailedError]({
...
//#endregion
exports.withRequiredClaims = withRequiredClaims;
