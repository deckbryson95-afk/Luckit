import { c as Errors, d as InvalidJwtError, f as JwksFetchFailedError, p as JwksNotConfiguredError } from "../../errors-DV038ZPa.mjs";
...
const withClaims = defineMiddleware({
	...
	run: (config) => async (req) => {
		...
		if (!jwks) return errorResponse(Errors[JwksNotConfiguredError]({ middleware: "withClaims" }), { errors: config?.errors });
