import { C as NoKeysConfiguredError, E as UnusableCredentialError, c as Errors, d as InvalidJwtError, f as JwksFetchFailedError, g as MissingCredentialsError, l as InvalidApiKeyError, p as JwksNotConfiguredError, t as AuthError, u as InvalidCredentialsError } from "./errors-DV038ZPa.mjs";
import { t as extractCredentials } from "./extract-credentials-BVHjxEOY.mjs";
import { i as diagnoseAuthorizationHeader, n as ApiKeyInAuthorizationHeader, r as apiKeyOnUserOnlyEndpoint, t as classifyApiKey } from "./classify-credentials-Lml480l-.mjs";
import { t as resolveEnv } from "./resolve-env-1gyyX-wv.mjs";
import { t as verifyUserJwt } from "./verify-user-jwt-C46esPLp.mjs";
	...
	switch (base) {
		...
		case "user": {
			...
			if (!verified.ok) {
				...
				return {
					kind: "reject",
					error: (context) => failure.kind === "jwks-source" ? Errors[JwksFetchFailedError]({
						...context,
...
function explainFallthrough(skips, context) {
	if (skips.some((skip) => skip.reason === "jwks-not-configured")) return Errors[JwksNotConfiguredError](context);
	const unreachableMode = skips.find((skip) => skip.reason === "no-keys-configured");
	if (unreachableMode) return Errors[NoKeysConfiguredError]({
		...context,
		mode: unreachableMode.mode,
		keyKind: unreachableMode.keyKind
	});
	const { authorization, apikey } = context.received;
	if (authorization === "absent" && apikey === "absent") return Errors[MissingCredentialsError](context);
	if (!context.authModes.some((mode) => mode === "publishable" || mode.startsWith("publishable:") || mode === "secret" || mode.startsWith("secret:")) && (authorization === "api-key" || apikey !== "absent")) return Errors[UnusableCredentialError]({
		...context,
	...
	if (authorization === "api-key" && apikey === "absent") return Errors[UnusableCredentialError]({
		...context,
		...ApiKeyInAuthorizationHeader
	});
	if (apikey !== "absent") {
		const misnamed = skips.find((skip) => skip.reason === "apikey-mismatch" && skip.matchedKeyName !== null);
		return Errors[InvalidApiKeyError](misnamed ? {
			...context,
			matchedKey: {
...
async function verifyAuth(request, options) {
	...
	const diagnosis = diagnoseAuthorizationHeader(request.headers.get("authorization"));
...
//#endregion
export { verifyCredentials as n, verifyAuth as t };
