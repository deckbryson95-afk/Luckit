...
const require_resolve_env = require("./resolve-env-DZIwNAkA.cjs");
const require_verify_user_jwt = require("./verify-user-jwt-DobukJGa.cjs");
...
* @internal
*/
async function tryMode(mode, credentials, env, options) {
		...
		case "secret": {
			...
			if (matched === null) return {
				...
				skip: {
					...
					mode,
					keyKind: base,
					matchedKeyName: keyName === "*" ? null : await matchApiKey(credentials.apikey, keys, "*")
				}
			};
			return {
				kind: "match",
...
function explainFallthrough(skips, context) {
	...
	if (apikey !== "absent") {
		const misnamed = skips.find((skip) => skip.reason === "apikey-mismatch" && skip.matchedKeyName !== null);
		return require_errors.Errors[require_errors.InvalidApiKeyError](misnamed ? {
			...context,
			matchedKey: {
